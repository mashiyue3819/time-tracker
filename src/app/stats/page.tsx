...
import { getCategories, getRecordsByDates } from "@/lib/db";
...
export default function StatsPage() {
  ...
  useEffect(() => {
    async function loadStats() {
      ...
      const [nextCategories, nextRecords] = await Promise.all([getCategories(), getRecordsByDates(dates)]);
  ...
  const stats = useMemo(() => aggregateByCategory(records, categories), [categories, records]);
    ...
    <AppShell activeTab="stats">
      <main className="safe-top pb-28">
        <header className="mb-5">
          ...
          <p className="mt-1 text-sm text-slate-500">按分类汇总最近的时间投入。</p>
        </header>
        ...
        <section className="mb-4 rounded-lg bg-white p-4 shadow-sm">
          ...
          <p className="mt-1 text-3xl font-semibold">{formatMinutes(totalMinutes)}</p>
        </section>
        ...
        {loading ? (
          ...
          <div className="space-y-3">
            {stats.map((item) => {
              const percent = totalMinutes > 0 ? Math.max(3, (item.minutes / totalMinutes) * 100) : 0;
              return (
                <article key={item.categoryId} className="rounded-lg bg-white p-4 shadow-sm">
                  <div className="mb-2 flex items-center justify-between gap-3">
                    ...
                    <span className="shrink-0 text-sm text-slate-500">{formatMinutes(item.minutes)}</span>
                  </div>
